...
class FlashCardTest(StageTest):
    def generate(self):
        return [
            ...
            TestCase(stdin=[self.test2_input1, self.test2_input2, self.test2_input3]),
            TestCase(stdin=["1", "4", self.test3_input1, self.test3_input2]),
            TestCase(stdin=["1", "1", " ", self.test4_input4, self.test4_input5]),
            TestCase(
                stdin=[
                    "1",
                    "1",
            ...
            TestCase(
                stdin=["2", self.test1_input10, self.test1_input11, self.test1_input12]
            ),
            TestCase(
                stdin=[
                    "2",
            ...
            TestCase(stdin=["2", self.test8_input2, self.test8_input3]),
        ]
    ...
    def check_question(self, out):
        if not "Question:" in out:
            raise WrongAnswerException(
                f"the word {out} spelling is wrong, it should be like this:\nQuestion:"
            )
        return True
    ...
    def check_practice_question(self, out, question):
        ...
        if len(out_list) < 2:
            ...
            if len(out_first_line) == 0:
                ...
                if len(out_list) < 4:
                    raise WrongAnswerException(
                        "It seems that the output for the question \n"
                        '"{0}"\n'
    ...
    def check_practice_answer(self, out, answer):
        ...
        if len(out_list) == 0:
            ...
            if len(first_part) == 0:
                ...
                if (
                    q_s_list[0] in out_list
                    and q_s_list[1] in out_list
                    and q_s_list[2] in out_list
                ):
                    return True
        ...
        if len(output_list) == 0:
            ...
            if len(first_line) < 2:
                ...
                try:
                    ...
                    output_list = "\n".join(output_list)
                    raise WrongAnswerException(
                        f"A wrong number of lines is found after the last input.\n"
                ...
                if not first_line[1].strip() == SECOND_QUESTION:
                    raise WrongAnswerException(
                        " after current question: question should be printed"
                    )
